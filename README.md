...
Перед прямой заменой `.app` Luma проверяет bundle identifier, версию и криптографическую подпись приложения через системный Code Signing Services. Для принимаемой подписи используется системное требование `anchor apple generic`, а проверка выполняется для всех архитектурных срезов. Если подпись недействительна или не соответствует требованию, артефакт отклоняется.
