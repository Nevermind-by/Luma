...
Перед прямой заменой `.app` Luma проверяет bundle identifier, версию и криптографическую подпись приложения через системный Code Signing Services. Используется требование `anchor apple generic`, поэтому принимаются Apple-issued code signing identities. Неподписанные, повреждённые или не соответствующие требованию приложения отклоняются до замены.
