...
struct AppsTorrentPageParserTests {
    ...
    @Test func parsesCurrentReleaseAndDownloadOptions() throws {
        ...
        #expect(release.distributionVariant == .standard)
    ...
    @Test func detectsMASDistributionVariant() throws {
        ...
        <span itemprop="softwareVersion">27.0.1-58670 [MAS]</span>
        <!--dle_spoiler Parallels Desktop 27.0.1-58670 [MAS] -->
        <a href="https://example.com/mas.pkg">Прямая ссылка</a>
        """
        let pageURL = URL(string: "https://appstorrent.ru/61-parallels-desktop.html")!

        let release = try parser.parse(html: html, pageURL: pageURL)

        #expect(release.distributionVariant == .mas)
        #expect(release.version == SoftwareVersion("27.0.1-58670 [MAS]"))
    }

    @Test func rejectsPageWithoutTitle() {
        let html = "<span itemprop=\"softwareVersion\">1.2.3</span>"
        let pageURL = URL(string: "https://appstorrent.ru/some-app.html")!

        #expect(throws: AppsTorrentPageParser.ParserError.missingTitle) {
    ...
    @Test func rejectsPageWithoutSupportedDownloadLinks() {
        let html = """
        <h1 itemprop="name">Some App</h1>
        <span itemprop="softwareVersion">1.2.3</span>
        <!--dle_spoiler Some App 1.2.3 -->
        <a href="https://example.com/unknown">Download mirror</a>
        """
        let pageURL = URL(string: "https://appstorrent.ru/some-app.html")!
